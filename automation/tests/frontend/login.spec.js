...
test.describe('Login', () => {
    ...
    test('deve logar com dados válidos e redirecionar para home', async ({loginPage, dadosLogin}) => {

        await loginPage.logar(dadosLogin.valido);
        await loginPage.validarLoginRealizado();
    });

    test('deve exibir erro ao tentar logar com credenciais inválidas', async ({loginPage, dadosLogin}) => {
    ...
    test('deve manter botão desabilitado com formulário vazio', async ({loginPage}) => {

        await loginPage.validarBotaoDesabilitado();
    });

    test('deve redirecionar para cadastro ao clicar em "Need an account?"', async ({loginPage}) => {
